...
if (@ARGV[0] eq "-f")
{
	...
	$DIR_INIT="${dir}:/TrafficSimulator/Test/OSM_temp/route";
}
else
{
	open (FILE, "< ./$file");
...
`rm $DIR_INIT/route_block_* 2>/dev/null`;
`rm $DIR_INIT/route_file_result_* 2>/dev/null`;
`rm $DIR_INIT/route_file_routed_* 2>/dev/null`;
#`rm $DIR_INIT/route_gps_points_* 2>/dev/null`;
...
for ($i = 0; $i < $NUM; $i++)
{
	...
	if ($index == $BLOCK)
	{
		...
		$count_txt= sprintf("%04d",$count_file);
		open (FILE, "> $DIR_INIT/route_block_${count_txt}_${file_name}.txt");
		$count_file++;
	}
	else
	{
